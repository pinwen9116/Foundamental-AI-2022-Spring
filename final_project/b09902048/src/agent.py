...
class MyPlayer(
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if sum(total_loss) < left_round * 10 * (-1):
            print("expect", sum(total_loss), left_round * 10 * (-1))
            return "fold", 0
        if next_cards == 0:
            next_cards = 1
    ...
    def receive_round_result_message(self, winners, hand_info, round_state):
        ...
        if not sum(total_loss) >=  left_round * (-10):
            network.fit(total_state, total_loss)
        # this is for Reinforce learnings
        #agent.forward(state)

        # save
        pickle.dump(network, open("./aagents/network_GBR.pkl", "wb"))
...
def setup_ai():
    # load trained data
    global network
    network = pickle.load(open("./aagents/network_GBR.pkl", "rb"))
